...
function init() {
  ...
  // Create slides for each image with alternating animation phases
  imageUrls.forEach(function(url, index) {
    var animationPhase = (index % 2 === 0) ? 'out' : 'in';
    var slide = new Slide(width, height, animationPhase);
    
    var loader = new THREE.ImageLoader();
    loader.setCrossOrigin('Anonymous');
    slide.setImage(loader.load(url));
    
    root.scene.add(slide);
    slides.push(slide);
  });

  // Add each slide's 3-second transition at 60-second intervals
  slides.forEach(function(slide, index) {
    tl.add(slide.transition(), index * 60);
  });
